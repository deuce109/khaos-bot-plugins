...
def _generate_random_numbers(command_string: str) -> str:
                ...
                return _random_numbers(**parsed_args)

def _random_numbers(min: int = 1, max: int = 100, digits: Optional[int] = None, amount: int = 1) -> str:
    if digits is not None:
        min = -int("9" * digits)
        max = int("9" * digits)
    
    return ",".join([str(random.randint(min, max)) for _ in range(amount)])
